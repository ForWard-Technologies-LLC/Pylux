#!/bin/bash
# tvOS build script for Pylux (Chiaki)
# tvOS counterpart of ios/build.sh: PLATFORM=TVOS, appletvos SDK, PyluxTV scheme
#
# ============================ HOW TO RUN + READ LOGS (tvOS) ============================
# Normal loop:   ./build.sh dev          # build + install + launch (sim or device) + start logs
# View logs:     ./build.sh logs         # one-shot tail -n 200 of tvos/logs/pylux.log (NEVER hangs)
# Stop logs:     ./build.sh stop-logs
#
# Every launch path (dev / launch / iterate) starts a BACKGROUND log capture into ONE fixed file
# (tvos/logs/pylux.log) and returns immediately -- nothing streams in the foreground, so it never
# hangs. To watch live instead of a snapshot:  tail -f tvos/logs/pylux.log
#
# APPLE TV LOGS WORK OVER WI-FI -- NO USB CABLE NEEDED (since 2026-07-08).
...
#   ./build.sh ship - Archive + export IPA + Fastlane upload (TestFlight). Uses generic/platform=tvOS only;
...
_pylux_start_phys_device_syslog_bg() {
    ...
    if ! idevice_id -l 2>/dev/null | grep -q . && ! idevice_id -ln 2>/dev/null | grep -q .; then
        ...
        echo "WARN: $log_file will stay EMPTY until the Apple TV is reachable." >&2
...
check_forbidden_inline_setters() {
    ...
    local hits
    hits=$(grep -rnE "($forbidden)\(" "$SCRIPT_DIR/PyluxTV" "$SCRIPT_DIR/../ios/Pylux" --include="*.m" --include="*.mm" --include="*.swift" 2>/dev/null | grep -v "_ex(" || true)
    if [ -n "$hits" ]; then
        echo "" >&2
...
if [ "$(uname -m)" = "arm64" ]; then
    ...
    SIMULATOR_PLATFORM="SIMULATOR_TVOS"
fi
...
build_lib() {
    drop_stale_ios_cmake_build "$SCRIPT_DIR/build-appletvos"
    drop_stale_ios_cmake_build "$SCRIPT_DIR/build-appletvsimulator"

    echo "=== Building chiaki-lib for tvOS device ==="
    cmake -S "$SCRIPT_DIR" -B "$SCRIPT_DIR/build-appletvos" -G Ninja \
        ...
        -DUSE_NGHTTP2=OFF \
        $CMAKE_EXTRA
    cmake --build "$SCRIPT_DIR/build-appletvos" --config Release --target chiaki-lib

    echo ""
    echo "=== Building chiaki-lib for tvOS simulator ($SIMULATOR_PLATFORM) ==="
    cmake -S "$SCRIPT_DIR" -B "$SCRIPT_DIR/build-appletvsimulator" -G Ninja \
        -DCMAKE_TOOLCHAIN_FILE="$TOOLCHAIN_FILE" \
        ...
        -DUSE_NGHTTP2=OFF \
        $CMAKE_EXTRA
    cmake --build "$SCRIPT_DIR/build-appletvsimulator" --config Release --target chiaki-lib

    echo ""
    echo "=== Creating combined libraries ==="
    create_combined_lib "$SCRIPT_DIR/build-appletvos" "$SCRIPT_DIR/build-appletvos/libchiaki_complete.a"
    create_combined_lib "$SCRIPT_DIR/build-appletvsimulator" "$SCRIPT_DIR/build-appletvsimulator/libchiaki_complete.a"
}
...
run_dev() {
    ...
    DESTINATIONS=$(xcodebuild -project "$SCRIPT_DIR/PyluxTV.xcodeproj" -scheme "$SCHEME" -showdestinations 2>/dev/null | grep "platform:tvOS," | grep -v "Simulator" | grep "name:" | _filter_real_ios_device_destinations)
    if [ -n "$DESTINATIONS" ]; then
        ...
        DEVICECTL_INFO=$(xcrun devicectl list devices 2>/dev/null | grep -E "Apple TV" | grep -v "unavailable" | grep -v "disconnected" | head -1)
    ...
    if [ -n "$DEVICE_UDID" ]; then
        ...
        touch "$SCRIPT_DIR/build-appletvos/libchiaki_complete.a"
        touch "$SCRIPT_DIR/build-appletvsimulator/libchiaki_complete.a"
        # Clean build folder to force relink
        rm -rf "$SCRIPT_DIR/build-derived/Build/Intermediates.noindex"
        echo ""
        echo "=== Building PyluxTV app for physical device ==="
        ...
        (cd "$SCRIPT_DIR" && xcodebuild -project PyluxTV.xcodeproj -scheme "$SCHEME" -sdk appletvos -configuration "$PYLUX_XCODE_CONFIGURATION" clean build \
            -destination "id=$DEVICE_UDID" \
            -derivedDataPath "$SCRIPT_DIR/build-derived" \
            -allowProvisioningUpdates \
            "${_PYLUX_XCODE_EXTRA[@]}")

        APP_PATH="$SCRIPT_DIR/build-derived/Build/Products/${PYLUX_XCODE_CONFIGURATION}-appletvos/PyluxTV.app"
        if [ ! -d "$APP_PATH" ]; then
        ...
        rm -rf "$SCRIPT_DIR/build-derived/Build/Intermediates.noindex"
        echo ""
        echo "=== Building PyluxTV app for simulator ==="
        ...
        (cd "$SCRIPT_DIR" && xcodebuild -project PyluxTV.xcodeproj -scheme "$SCHEME" -sdk appletvsimulator -configuration "$PYLUX_XCODE_CONFIGURATION" clean build \
            -destination 'generic/platform=tvOS Simulator' \
            "$XC_ARCHS" \
            -derivedDataPath "$SCRIPT_DIR/build-derived" \
            "${_PYLUX_XCODE_EXTRA[@]}")

        APP_PATH="$SCRIPT_DIR/build-derived/Build/Products/${PYLUX_XCODE_CONFIGURATION}-appletvsimulator/PyluxTV.app"
        if [ ! -d "$APP_PATH" ]; then
            echo "ERROR: App not found at $APP_PATH"
            exit 1
        fi
        ...
        if [ -z "$BOOTED" ]; then
            ...
            echo "  xcrun simctl boot 'Apple TV'"
            echo ""
...
ARCHIVE_PATH="$SCRIPT_DIR/build-derived/PyluxTV.xcarchive"
...
create_release_archive() {
    ...
    (cd "$SCRIPT_DIR" && xcodebuild -project PyluxTV.xcodeproj -scheme "$SCHEME" -sdk appletvos -configuration Release archive \
    ...
    if [ ! -d "$ARCHIVE_PATH" ]; then
        ...
        echo "  Open PyluxTV.xcodeproj > Signing & Capabilities > select your Team"
        exit 1
    fi
    echo ""
...
run_ship() {
    ...
    (cd "$SCRIPT_DIR/../ios" && FASTLANE_PLATFORM_NAME=tvos fastlane tvos upload_pylux_ipa)
    echo ""
    echo "Done. When processing finishes, the build appears in App Store Connect → TestFlight."
}

create_xcframework() {
    COMBINED_DEVICE="$SCRIPT_DIR/build-appletvos/libchiaki_complete.a"
    COMBINED_SIM="$SCRIPT_DIR/build-appletvsimulator/libchiaki_complete.a"
    FRAMEWORK_DIR="$SCRIPT_DIR/PyluxTV.xcframework"
    rm -rf "$FRAMEWORK_DIR"
    mkdir -p "$FRAMEWORK_DIR/tvos-arm64/chiaki.framework"
    mkdir -p "$FRAMEWORK_DIR/tvos-arm64_x86_64-simulator/chiaki.framework"
    cp "$COMBINED_DEVICE" "$FRAMEWORK_DIR/tvos-arm64/chiaki.framework/chiaki"
    cp "$COMBINED_SIM" "$FRAMEWORK_DIR/tvos-arm64_x86_64-simulator/chiaki.framework/chiaki"
    cat > "$FRAMEWORK_DIR/tvos-arm64/chiaki.framework/Info.plist" << 'PLIST'
...
PLIST
    cp "$FRAMEWORK_DIR/tvos-arm64/chiaki.framework/Info.plist" "$FRAMEWORK_DIR/tvos-arm64_x86_64-simulator/chiaki.framework/"
...
run_launch() {
    ...
    DEVICE_INFO=$(xcrun devicectl list devices 2>/dev/null | grep -E "Apple TV" | grep -v "unavailable" | grep -v "disconnected" | head -1)
    ...
    if [ -n "$DEVICE_INFO" ]; then
        ...
        launch_phys_with_console_log_bg "$DEVICE_UDID" "$BUNDLE_ID"
        echo ""
        echo "App launched on physical device: $DEVICE_NAME"
    else
...
clean_and_rebuild() {
    ...
    if [ -d "$SCRIPT_DIR/build-appletvos" ]; then
        echo "Removing build-appletvos..."
        rm -rf "$SCRIPT_DIR/build-appletvos"
    fi
    
    if [ -d "$SCRIPT_DIR/build-appletvsimulator" ]; then
        echo "Removing build-appletvsimulator..."
        rm -rf "$SCRIPT_DIR/build-appletvsimulator"
    fi
    ...
    xcodebuild clean -project "$SCRIPT_DIR/PyluxTV.xcodeproj" -scheme "$SCHEME" -configuration "$PYLUX_XCODE_CONFIGURATION" 2>&1 | grep -E "^(Clean|note:|error:)" || true
...
run_iterate() {
    ...
    if [ -z "$DEVICE_UDID" ]; then
        echo "ERROR: No physical Apple TV for xcodebuild. Connect an Apple TV and trust this Mac."
        exit 1
    fi
    INSTALL_UDID="${DEVICECTL_UDID:-$DEVICE_UDID}"
    ...
    touch "$SCRIPT_DIR/build-appletvos/libchiaki_complete.a" 2>/dev/null || true
    touch "$SCRIPT_DIR/build-appletvsimulator/libchiaki_complete.a" 2>/dev/null || true

    echo "=== Xcode build (incremental, no clean) ==="
    (cd "$SCRIPT_DIR" && xcodebuild -project PyluxTV.xcodeproj -scheme "$SCHEME" -sdk appletvos -configuration "$PYLUX_XCODE_CONFIGURATION" build \
        -destination "id=$DEVICE_UDID" \
        -derivedDataPath "$SCRIPT_DIR/build-derived" \
        -allowProvisioningUpdates \
        "${_PYLUX_XCODE_EXTRA[@]}") || {
