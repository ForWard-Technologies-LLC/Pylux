...
Added Touchpad support for touchscreens (use outer edges of touchscreen for touchpad click) as well as mouse input (enables trackpads to be fully supported with gestures on touch and touchpad click on click). For an example mapping these in a Steam Deck controller profile, see [Default `pylux` Layout Touchscreen Mapping](../setup/controlling.md#default-Pylux-layout-touchscreen-mapping){target="_blank" rel="noopener"} and [Default `pylux` Layout Trackpad Mapping](../setup/controlling.md#default-Pylux-layout-trackpad-mapping){target="_blank" rel="noopener"} respectively. Downloading the updated `pylux+` controller layout will have these mappings set for your convenience.[^1]
...
You can now attach a microphone and use it to chat online or use in games. There is also noise suppression and echo cancelling for the mic you can configure in the pylux settings.[^1]
...
Haptics enabled for PlayStation 5 thanks to [Johannes Baiter](https://github.com/jbaiter){target="_blank" rel="noopener"}. You can use a USB connected DualSense for haptics and adaptive triggers or a bluetooth connected DualSense for just adaptive triggers. To use these features for the DualSense in game mode, please disable Steam Input for the DualSense controller following the "Turning off Steam Input" tab in [this section](../setup/controlling.md#enabling-Pylux-to-work-with-dualsense-dualshock-4){target="_blank" rel="noopener"}.[^2].
...
It's great to have these options for Steam Deck's non-standard 1280x800 resolution.[^3]
    ...
    1. Adjust the window to a bigger or smaller size with your mouse in `Desktop Mode` (perhaps you want to use `pylux` as half of your screen in Desktop mode)
...
This makes it as easy as possible for Steam Deck users to use touchpad and gyro support with their DualSense or DualShock 4 controller. See [Using a DualSense and/or DualShock4 Controller with Pylux](../setup/controlling.md#using-a-dualsense-andor-dualshock4-controller-with-Pylux){target="_blank" rel="noopener"} to set it up for yourself.[^3]
The DualSense/DualShock 4 touchpad and gyro controls work with the binary version of Chiaki but don't work with the official flatpak version. Luckily for you, they now work with the `pylux` flatpak! :smile:
