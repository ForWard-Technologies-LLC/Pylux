...
static bool always_false_cond_check(void *user)
{
	(void)user;
	return false;
}
...
#define QUIT(quit_label) do { \
	...
	// Ping/MTU (Senkusha): Cloud uses protocol version 9. Remote Play behavior unchanged.
	// Store protocol version in senkusha for use in senkusha_set_version
	// Only set if not already set (allows external code to override)
	if(senkusha->protocol_version == 0) {
		senkusha->protocol_version = chiaki_service_type_is_cloud(session->service_type) ? 9 : 7;
	}
...
static void senkusha_takion_data(ChiakiSenkusha *senkusha, ChiakiTakionMessageDataType data_type, uint8_t *buf, size_t buf_size)
{
	...
	chiaki_mutex_lock(&senkusha->state_mutex);
	bool expecting_big_echo = (senkusha->state == STATE_EXPECT_BIG_ECHO_COMPLETE);
	chiaki_mutex_unlock(&senkusha->state_mutex);
	...
	if(!r)
	{
		// For cloud mode, if we're expecting the echoed BIG message and decode fails,
		// we need to wait for all chunks to be received and reassembled.
		// The echoed BIG message should be large (similar to what we sent, ~2000+ bytes).
		// Only treat it as complete when we receive a message that's large enough
		// to indicate all chunks have been reassembled.
		if(expecting_big_echo && buf_size > 0)
		{
			// The complete echoed BIG message should be large (we sent ~2000+ bytes)
			// Individual chunks are ~550 bytes, so we need to wait for the complete message
			// Only treat it as complete when we receive a message that's large enough
			// (e.g., > 1500 bytes) to indicate all chunks are reassembled
			if(buf_size > 1500) // Complete echoed BIG messages are large (all chunks reassembled)
			{
				CHIAKI_LOGV(senkusha->log, "Senkusha received complete echoed BIG message (%zu bytes, all chunks reassembled)", buf_size);
				chiaki_log_hexdump(senkusha->log, CHIAKI_LOG_VERBOSE, buf, buf_size > 64 ? 64 : buf_size);
				
				chiaki_mutex_lock(&senkusha->state_mutex);
				if(senkusha->state == STATE_EXPECT_BIG_ECHO_COMPLETE)
				{
					CHIAKI_LOGV(senkusha->log, "Senkusha detected complete echoed BIG message (all chunks ACKed)");
			...
			{
				// This is likely a partial chunk, wait for more
				CHIAKI_LOGD(senkusha->log, "Senkusha received partial echoed BIG chunk (%zu bytes), waiting for complete message", buf_size);
			}
		}
		CHIAKI_LOGE(senkusha->log, "Senkusha failed to decode data protobuf (buf_size=%zu)", buf_size);
		if(expecting_big_echo)
		{
			CHIAKI_LOGV(senkusha->log, "Senkusha protobuf decode failed for echoed BIG, dumping buffer:");
			chiaki_log_hexdump(senkusha->log, CHIAKI_LOG_VERBOSE, buf, buf_size > 128 ? 128 : buf_size);
		}
		return;
	}
	...
	else if(senkusha->state == STATE_EXPECT_BIG_ECHO_COMPLETE)
	{
		// For cloud mode: wait for the echoed BIG message (server echoes back our BIG message)
		// The takion layer has already ACKed all chunks, so we just need to detect the complete message
		if(msg.type == tkproto_TakionMessage_PayloadType_BIG && msg.has_big_payload)
		{
			CHIAKI_LOGV(senkusha->log, "Senkusha received complete echoed BIG message (all chunks ACKed)");
			senkusha->state_finished = true;
			chiaki_cond_signal(&senkusha->state_cond);
		}
		// Ignore other message types (they might be chunks being reassembled)
	}
...
static ChiakiErrorCode senkusha_set_version(ChiakiSenkusha *senkusha)
{
	...
	memset(&msg, 0, sizeof(msg));
	List versions;
	// Use the protocol version stored in senkusha (set in chiaki_senkusha_run)
	versions.items[0] = senkusha->protocol_version;
